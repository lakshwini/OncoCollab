...
export function VideoConferenceAdvanced({
  ...
  const refreshPrerequisitesPanel = useCallback(async () => {
    if (!roomId) {
      return;
    }

    try {
      const data = await fetchMeetingPrerequisiteDetails(roomId, authToken ?? null);
      setPrereqData(data);

      if (activePrereqDoctor && activePrereqItem) {
        const nextDoctor = data.participants.find(
          (participant) => participant.doctor_id === activePrereqDoctor.doctor_id,
        );
        const nextItem = nextDoctor?.items.find((item) => item.key === activePrereqItem.key) || null;
        setActivePrereqDoctor(nextDoctor || null);
        setActivePrereqItem(nextItem);
      }
    } catch (err) {
      console.error('[VideoConf] ❌ Erreur refresh manuel prérequis:', err);
    }
  }, [roomId, authToken, activePrereqDoctor, activePrereqItem]);
          ...
          {viewMode === 'prerequisites' ? (
            ...
            activePrereqItem && activePrereqDoctor ? (
              <PrerequisiteModulePlaceholder
                ...
                onSaved={() => {
                  void refreshPrerequisitesPanel();
                }}
              />
            ) : (
              <div style={{
