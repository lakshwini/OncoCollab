...
export class MeetingsService {
  ...
  private async getDoctorSpecialities(doctorIds: string[]): Promise<Map<string, string>> {
    ...
    const placeholders = uniqueDoctorIds.map((_, index) => `$${index + 1}`).join(', ');
    const doctors = await this.dataSource.query(
      `SELECT
       ...
       WHERE d.doctorid IN (${placeholders})`,
      uniqueDoctorIds,
    );

    return new Map<string, string>(
      doctors.map((doctor: { doctorId: string; speciality: string }) => [doctor.doctorId, doctor.speciality]),
    );
  }
  ...
  async createMeeting(createMeetingDto: any, createdBy: string): Promise<{ meetingId: string }> {
    ...
    // 5. Stocker les prérequis manuels de la réunion dans MongoDB
    if (participants && participants.length > 0) {
      const doctorSpecialities = await this.getDoctorSpecialities(
        participants.map((participant: { doctorId: string }) => participant.doctorId),
      );

      const prerequisitesByDoctor = new Map<string, any>(
        Array.isArray(prerequisites)
          ? prerequisites
              .filter((entry: any) => entry && typeof entry.doctorId === 'string')
              .map((entry: any) => [entry.doctorId, entry])
          : [],
      );

      const doctors = participants.map((participant: { doctorId: string }) => {
        const manualDoctor = prerequisitesByDoctor.get(participant.doctorId);
        const manualItems = Array.isArray(manualDoctor?.items) ? manualDoctor.items : [];

        return {
          doctor_id: participant.doctorId,
          speciality:
            (typeof manualDoctor?.speciality === 'string' && manualDoctor.speciality.trim().length > 0
              ? manualDoctor.speciality
              : doctorSpecialities.get(participant.doctorId)) || 'Non spécifié',
          items: manualItems
            .filter((item: any) => item && typeof item.key === 'string')
            .map((item: any) => ({
              key: item.key,
              label: item.label || item.key,
              label_fr: item.label_fr || item.label || item.key,
              label_en: item.label_en || item.label || item.key,
