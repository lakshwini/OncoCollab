...
export class PrerequisitesService {
  ...
  /**
   * Récupère les prérequis d'une réunion depuis MongoDB
   * Format de réponse:
   * {
   *   meeting_id,
   ...
   * }
   */
  async getMeetingPrerequisitesByMeetingId(meetingId: string) {
    ...
    return {
      ...
      doctors: (prerequisites.doctors || []).map((doctor: any) => ({
      ...
      meetings.map(async (meeting: any) => {
        try {
          ...
          return {
            ...
            is_admin: isAdmin,
            speciality: doctorData?.speciality || 'Non spécifié',
            prerequisites,
          };
        } catch (err) {
  ...
  /**
   * Met à jour le statut d'un prérequis avec support 3-états (pending, in_progress, done)
   * Utilisé par PrerequisitesPreparationPage.tsx
   */
  async updatePrerequisiteStatus(
    meetingId: string,
    doctorId: string,
    itemId: string,
    ...
    await this.mongoDb.collection('meeting_prerequisites').updateOne(
      ...
      {
        $set: {
          'doctors.$[doctor].items.$[item].status': status,
          'doctors.$[doctor].items.$[item].updatedAt': new Date(),
          updatedAt: new Date(),
        },
      },
      ...
      await this.mongoDb.collection('meeting_prerequisites').updateOne(
        ...
        },
        {
          $set: {
            ...
            'doctors.$[doctor].items.$[item].value': item.value ?? null,
          },
        },
        {
          arrayFilters: [
